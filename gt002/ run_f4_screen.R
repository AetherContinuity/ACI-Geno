...
# Merge: replace population with custom_pop_label
mod_ind <- merge(orig_ind, popmap[, .(sample_id, custom_pop_label)],
                 by = "sample_id", all.x = TRUE)
mod_ind[, population := fifelse(is.na(custom_pop_label),
                                 "UNUSED", custom_pop_label)]
mod_ind[, custom_pop_label := NULL]
...
# 2. EXTRACT f2 BLOCKS
# ==========================================================================
cat("[2/6] Extracting f2 blocks (may take 20-60 min)...\n")

analysis_pops <- unique(popmap$custom_pop_label)
...
extract_f2(
  ...
  maxmem    = 12000,    # use up to 12 GB RAM (runner has 16 GB)
  verbose   = TRUE
)
...
# ADMIXTOOLS2 f4() with comb = FALSE takes equal-length vectors and
# computes row-by-row: f4(pop1[i], pop2[i]; pop3[i], pop4[i]).
# This is far faster than looping (loads f2 data once).
# Returns a tibble: pop1, pop2, pop3, pop4, est, se, z, p.
results <- tryCatch({
  f4(f2_blocks,
     ...
     comb = FALSE,
     sure = TRUE)  # suppress safety check for >10000 combos
}, error = function(e) {
  cat("  Batch f4 failed, falling back to loop mode...\n")
  ...
  for (i in seq_len(nrow(f4_design_run))) {
    ...
    r <- tryCatch({
      f4(f2_blocks,
         pop1 = as.character(t$pop1), pop2 = as.character(t$pop2),
         pop3 = as.character(t$pop3), pop4 = as.character(t$pop4),
         comb = FALSE)
